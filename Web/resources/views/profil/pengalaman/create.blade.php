...
@section('content')
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating mb-3">
                                        <select name="is_current" id="is_current"
                                            class="form-select @error('is_current') is-invalid @enderror" required>
                                            <option value="" disabled selected>Pilih status pekerjaan</option>
                                            <option value="1" {{ old('is_current') == '1' ? 'selected' : '' }}>Masih
                                                bekerja di sini</option>
                                            <option value="0" {{ old('is_current') == '0' ? 'selected' : '' }}>Sudah
                                                tidak bekerja di sini</option>
                                        </select>
                                        <label for="is_current"><i class="fas fa-briefcase me-2"></i>Status
                                            Pekerjaan</label>
                                        @error('is_current')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Optionally, if you want to handle logic for is_current dropdown (not necessary but possible):
            const currentJobDropdown = document.getElementById('is_current');
            currentJobDropdown.addEventListener('change', function() {
                // Optionally handle the change event if needed
                // You can access the selected value here (1 for still working, 0 for not working)
                const status = currentJobDropdown.value;
                console.log('Job status selected:', status);
            });
        }); 
    </script>
@endsection
